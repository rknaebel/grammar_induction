...
def calcStdScore(parse_filename, gold_filename):
    ...
    return gold_count, parse_count, match_count

def calcExactScore(parse_filename, gold_filename):
    match_count = parse_count = gold_count = 0
    for parse_line, gold_line in zip(open(parse_filename), open(gold_filename)):
        gold = tree.Tree.from_str(gold_line)
        parse = tree.Tree.from_str(parse_line)
        if str(gold): gold_count += 1
        if str(parse): parse_count += 1
        if gold == parse: match_count += 1
    return gold_count, parse_count, match_count

def main(parse_filename, gold_filename):
    gold_count, parse_count, match_count = calcScore(parse_filename, gold_filename)
